...
# membuka file pickle
with open("datanya/used_data.pickle", 'rb') as f:
    data = pickle.load(f)
...
categories = ["All Categories"] + \
    list(data['category'].value_counts().keys().sort_values())
...
fig = px.bar(bar_data, color=bar_data, orientation='h',
             title=f'Channel Terpopuler dari kategori {category}')
st.plotly_chart(fig)

# visal dengan scatte plot
...
fig = px.scatter(outputs, x=choice_1, y=choice_2, size='view', hover_name='channel_name', hover_data=[
                 'title'], title=f'Engagement of {choice_1.title()} and {choice_2.title()}')
st.plotly_chart(fig)
